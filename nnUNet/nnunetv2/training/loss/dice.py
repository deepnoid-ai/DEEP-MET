...
class TverskyPlusLoss(nn.Module):
    """
    Tversky-plus loss (same interface/output convention as SoftDiceLoss, i.e.
    returns the negative aggregated index so lower = better).

        Tversky_index(c) = TP_c / (TP_c + (1-delta)*FP_c + delta*FN_c)

    - ``delta`` (default 0.7) weights FN over FP -> recall-oriented (good for
      small / sparse / under-segmented structures).
    - ``gamma`` (default 2.0) is the "plus" focal modulation applied at the
      voxel level inside get_tp_fp_fn_tn (fp -> fp**gamma, fn -> fn**gamma),
      which down-weights soft/uncertain errors and focuses on confident ones.

    Drop-in replacement for SoftDiceLoss in the compound losses.
    """
    def __init__(self, apply_nonlin: Callable = None, batch_dice: bool = False, do_bg: bool = True,
                 smooth: float = 1., ddp: bool = True, clip_tp: float = None,
                 gamma: float = 2., delta: float = 0.7):
        super(TverskyPlusLoss, self).__init__()
    ...
    def forward(self, x, y, loss_mask=None):
        ...
        # plus=True -> voxel-level focal modulation (fp**gamma, fn**gamma)
        tp, fp, fn, _ = get_tp_fp_fn_tn(x, y, axes, loss_mask, False, True, self.gamma)
        ...
        nominator = tp
        denominator = tp + fp * (1 - self.delta) + fn * (self.delta)
...
def get_tp_fp_fn_tn(net_output, gt, axes=None, mask=None, square=False, plus=False, gamma=2.0):
    """
    ...
    # "plus" focal modulation (Tversky-plus): raise per-voxel fp/fn to gamma
    # BEFORE summation, down-weighting soft/uncertain errors. tp is left as-is.
    if plus:
        fp = fp ** gamma
        fn = fn ** gamma
