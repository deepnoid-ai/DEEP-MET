...
class nnUNetDataLoader(DataLoader):
    def __init__(self,
        ...
        # --- Optional class-aware foreground oversampling (set via env by run_training.py). ---
        # When a forced-foreground patch is sampled, the class/region to center the patch on is
        # normally drawn UNIFORMLY among the classes present in the case. Here we can bias that
        # draw toward chosen labels (e.g. RC=4) so under-represented classes appear centered more
        # often -- WITHOUT duplicating whole cases, so case diversity is preserved.
        self.fg_oversample_enabled = os.environ.get('nnUNet_fg_oversample', '0') in ('1', 'True', 'true')
        self.fg_oversample_weights = {}
        if self.fg_oversample_enabled:
            spec = os.environ.get('nnUNet_fg_oversample_weights', '').strip()
            for tok in spec.split(','):
                ...
                lab, w = tok.split(':', 1)
                try:
                    self.fg_oversample_weights[int(lab)] = float(w)
                except ValueError:
                    pass

    def _fg_class_probabilities(self, eligible_classes_or_regions):
        """Probability vector over eligible classes/regions for class-aware foreground
        oversampling. A key matching a boosted label (int key == label, or label contained
        in a tuple/region key) gets its configured weight; everything else gets weight 1.0.
        Returns None (-> uniform) if weights are unusable."""
        weights = []
        for key in eligible_classes_or_regions:
            if isinstance(key, tuple):
                w = max([self.fg_oversample_weights.get(int(l), 1.0) for l in key])
            else:
                w = self.fg_oversample_weights.get(int(key), 1.0)
        ...
        if not force_fg and not self.has_ignore:
            ...
            if not force_fg and self.has_ignore:
                ...
                if len(eligible_classes_or_regions) == 0:
                    ...
                    if overwrite_class is not None and overwrite_class in eligible_classes_or_regions:
                        selected_class = overwrite_class
                    elif self.fg_oversample_enabled and self.fg_oversample_weights:
                        # class-aware (weighted) foreground oversampling
                        p_choice = self._fg_class_probabilities(eligible_classes_or_regions)
                        selected_class = eligible_classes_or_regions[
                            np.random.choice(len(eligible_classes_or_regions), p=p_choice)]
                    else:
