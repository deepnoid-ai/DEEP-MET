...
from nnunetv2.training.data_augmentation.custom_transforms.lla_transforms import MaskRegionLocalLocationScaleTransform
...
from nnunetv2.training.loss.compound_losses import DC_and_CE_loss, DC_and_BCE_loss, \
    DC_CE_and_TverskyPlus_loss, DC_BCE_and_TverskyPlus_loss
...
class nnUNetTrainer(object):
    ...
    def _build_loss(self):
        # --- Optional TverskyPlus loss (replaces the Dice term). Toggled via env var. ---
        use_tp = os.environ.get("nnUNet_tversky_plus", "0") in ("1", "True", "true")
        tp_delta = float(os.environ.get("nnUNet_tp_delta", "0.7"))
        tp_gamma = float(os.environ.get("nnUNet_tp_gamma", "2.0"))
        tp_weight = float(os.environ.get("nnUNet_tp_weight", "1.0"))

        if self.label_manager.has_regions:
            if use_tp:
                loss = DC_BCE_and_TverskyPlus_loss(
                    {},
                    {'batch_dice': self.configuration_manager.batch_dice,
                     'do_bg': True, 'smooth': 1e-5, 'ddp': self.is_ddp,
                     'delta': tp_delta, 'gamma': tp_gamma},
                    weight_ce=1, weight_tversky=tp_weight,
                    use_ignore_label=self.label_manager.ignore_label is not None)
                self.print_to_log_file(
                    f"[Loss] BCE + TverskyPlus(delta={tp_delta}, "
                    f"gamma={tp_gamma}, w_tversky={tp_weight})")
            else:
            ...
            if use_tp:
                loss = DC_CE_and_TverskyPlus_loss(
                    {},
                    {'batch_dice': self.configuration_manager.batch_dice,
                ...
                self.print_to_log_file(
                    f"[Loss] CE + TverskyPlus(delta={tp_delta}, "
                    f"gamma={tp_gamma}, w_tversky={tp_weight})")
            else:
        ...
        if self._do_i_compile():
            if getattr(loss, 'dc', None) is not None:
                loss.dc = torch.compile(loss.dc)
            if getattr(loss, 'tversky', None) is not None:
    ...
    def get_training_transforms(
        ...
        )  # p_... default 값 0.2 (p_elastic_deform은 0)
        ###
        ...
        ### NOTE: SLAug LLA (mask mode) — toggled via env vars set by run_training.py.
        # Per-(label-)region location-scale intensity augmentation (x -> a*x + b), the
        # original-paper mask variant. Enabled with --slaug_lla --slaug_lla_mode mask.
        if os.environ.get("nnUNet_slaug_lla", "0") in ("1", "true", "True"):
            lla_scale_range = (float(os.environ.get("nnUNet_slaug_lla_scale_min", "0.8")),
                               float(os.environ.get("nnUNet_slaug_lla_scale_max", "1.2")))
            lla_shift = float(os.environ.get("nnUNet_slaug_lla_shift", "0.2"))
            lla_shift_range = (-lla_shift, lla_shift)
            transforms.append(RandomTransform(
                MaskRegionLocalLocationScaleTransform(
                    p_per_channel=1.0,
                    synchronize_channels=False,
                    scale_range=lla_scale_range,
                    shift_range=lla_shift_range,
                ),
                apply_probability=float(os.environ.get("nnUNet_slaug_lla_p", "0.2"))
            ))
        ###
    ...
    def on_epoch_end(self):
        ...
        self.print_to_log_file('train_loss', round(float(self.logger.get_value('train_losses', step=-1)), 4))
        self.print_to_log_file('val_loss', round(float(self.logger.get_value('val_losses', step=-1)), 4))
        self.print_to_log_file('Pseudo dice', [round(float(i), 4) for i in
                                               self.logger.get_value('dice_per_class_or_region', step=-1)])
        self.print_to_log_file(
            f"Epoch time: {round(float(self.logger.get_value('epoch_end_timestamps', step=-1) - self.logger.get_value('epoch_start_timestamps', step=-1)), 2)} s")
